...
function nodesRouterFactory (
    ...
    router.post('/nodes/:identifier/workflows', rest(function(req) {
        ...
        return Promise.all([
            waterline.nodes.needByIdentifier(req.params.identifier),
            workflowApiService.findGraphDefinitionByName(configuration.name)
        ])
        .spread(function (node, definition) {
            var context = _.defaults(configuration.context || {}, { target: node.id });
            return [node.id,
                    workflowApiService.createGraph(
                                definition,
