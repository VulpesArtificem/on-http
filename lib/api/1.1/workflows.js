...
function workflowsRouterFactory (
    ...
    router.post('/workflows/active', rest(function(req) {
        var configuration = _.defaults(req.query || {}, req.body || {});

        return workflowApiService.findGraphDefinitionByName(configuration.name)
        .then(function (definition) {
            return workflowApiService.createGraph(
                        definition, configuration.options, {}, configuration.domain);
        })
        .then(function(graph) {
            workflowApiService.runTaskGraph(
    ...
    /**
     * @api {delete} /api/1.1/workflows DELETE /
     * @apiVersion 1.1.0
     * @apiDescription cancel an active workflow
     * @apiName workflows-cancel
     * @apiGroup workflows
     * @apiError Error problem was encountered, workflow was not canceled.
     */

    router.delete('/workflows/active/:identifier', rest(function(req) {
        return workflowApiService.deleteTaskGraph(req.params.identifier);
    }));
    ...
    router.get('/workflows/tasks/library', rest(function (req) {
        ...
        return waterline.taskdefinitions.find(filter);
    }));
    ...
    router.get('/workflows/library/:identifier', rest(function (req) {
        ...
        return waterline.graphdefinitions.find(filter);
    }));
