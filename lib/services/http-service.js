...
function httpServiceFactory(
    ...
    HttpService.prototype.createSwagger = function() {
        ...
        var upgradeConfig = {
            appRoot: path.resolve(__dirname, '../../'),
            configDir: path.resolve(__dirname, '../../swagger_config'),
            swagger: path.resolve(__dirname, '../../static/upgrade.yaml'),
            authEnabled: this.endpoint.authEnabled
        };
        ...
        [swaggerConfig, redfishConfig, sbSwaggerConfig].map(
            this.patchYamlConfig.bind(null, this.endpoint));
        ...
        var upgrade = swaggerCreateAsync(upgradeConfig).then(function(swaggerExpress) {
            return swaggerExpress.register(app);
        });
        ...
        return Promise.all([apiV2, sbApiV2, upgrade, redfish]);
    };
